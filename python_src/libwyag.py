...
def tree_parse_one(raw: bytes, start: int = 0) -> Tuple[int, "GitTreeLeaf"]:
    """
    Parse a single entry from a Git tree object.

    This function reads one tree entry starting at a given byte offset
    in the raw tree object data and returns both the position immediately
    after the entry and a `GitTreeLeaf` representing the parsed result.

    A Git tree entry has the following binary layout:

        <mode><space><path><NUL><20-byte SHA>

    Where:
        - mode is an ASCII-encoded file mode (5 or 6 bytes)
        - path is a UTF-8 encoded file or directory name
        - NUL is the byte 0x00
        - SHA is a 20-byte binary object ID

    Args:
        raw (bytes): Raw contents of a Git tree object.
        start (int, optional): Byte offset at which the tree entry begins.
            Defaults to 0.

    Returns:
        tuple:
            - int: Byte offset immediately after this tree entry.
            - GitTreeLeaf: Parsed tree entry containing mode, path, and SHA.

    Raises:
        AssertionError: If the mode length is not 5 or 6 bytes, indicating
            a malformed tree entry.

    Example:
        >>> offset, leaf = tree_parse_one(raw_tree_bytes)
        >>> leaf.mode
        b'100644'
        >>> leaf.path
        'README.md'
...
def tree_parse(raw: bytes) -> List["GitTreeLeaf"]:
    """
    Parse all entries from a Git tree object.

    This function iteratively parses the raw byte contents of a Git tree
    object into a list of `GitTreeLeaf` entries. Each entry represents a
    file or subdirectory stored in the tree, including its mode, path,
    and object SHA.

    Parsing proceeds sequentially from the beginning of the byte stream
    until all data has been consumed. Internally, this function relies
    on `tree_parse_one` to parse individual tree entries.

    Args:
        raw (bytes): Raw contents of a Git tree object.

    Returns:
        list[GitTreeLeaf]: A list of parsed tree entries in the order
        they appear in the tree object.

    Example:
        >>> leaves = tree_parse(raw_tree_bytes)
        >>> leaves[0].path
        'README.md'
        >>> leaves[0].mode
        b'100644'
